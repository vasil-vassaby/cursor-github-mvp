...
- **Mock‑режим** (по умолчанию): если `OPENAI_API_KEY` не задан, endpoint
  `/generate` возвращает предсказуемый mock‑текст на основе входных
  параметров.
- **Real‑режим**: если `OPENAI_API_KEY` задан, backend пытается вызвать
  ChatGPT через официальный OpenAI Python SDK v1 и вернуть сгенерированный
  текст. При ошибке AI‑провайдера backend мягко возвращается к mock‑тексту
  с префиксом в `result`, что это mock.

Используемые переменные окружения (значения задаются в корневом `.env`,
в репозиторий не коммитятся):

- `OPENAI_API_KEY` — обязательна для real‑режима. Без неё всегда mock.
- `OPENAI_BASE_URL` — опционально, по умолчанию `https://api.openai.com/v1`.
- `OPENAI_MODEL` — опционально, по умолчанию `gpt-4.1-mini` или другой
  разумный модельный идентификатор.
- `OPENAI_TEMPERATURE` — опционально, температура выборки (default: `0.7`).
- `OPENAI_TIMEOUT_SECONDS` — опционально, таймаут HTTP‑запроса к AI
  (default: `30` секунд).

Переключение режимов:

- убрать/не задавать `OPENAI_API_KEY` → всегда mock‑режим;
- задать `OPENAI_API_KEY` (и, при необходимости, другие `OPENAI_*`) →
  попытка реального вызова, с fallback обратно в mock при ошибках AI.
...
## Изменения в feature/prompt-quality-and-debug

- Уточнён шаблон промпта (`buildPrompt`): добавлены хуки под разные типы
  текста, учёт тона и длины, строгая структура заголовков/основного
  текста/CTA/блока «Важно».
- Улучшен mock‑ответ backend `/generate`, чтобы он ближе соответствовал
  новой структуре (подзаголовки, единый основной текст, мягкие CTA и
  аккуратный дисклеймер).
- Расширен debug‑mode на фронтенде: промпт отображается в отдельном
  блоке с кнопкой «Копировать промпт» для быстрого переноса в другие
  инструменты.
- Улучшен UX формы: подсказка о незаполненных обязательных полях,
  отдельная кнопка «Очистить форму», более аккуратная обработка ошибок
  в случае валидации на backend.
- Добавлена возможность быстро сбросить результат на панели результата
  через кнопку «Очистить текст», не затрагивая введённые данные формы.
